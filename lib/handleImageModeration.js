...
const handleImageModeration = async ({
  ...
  try {
    ...
    console.log(process.env.NEXT_PUBLIC_IMAGE_MODERATION_URL)
    const response = await axios.get(process.env.NEXT_PUBLIC_IMAGE_MODERATION_URL, {
      params: {
        url,
        models: 'nudity-2.0,wad,offensive,scam,gore,qr-content',
        api_user: process.env.MODERATION_API_USER,
        api_secret: process.env.MODERATION_API_SECRET,
      },
    });
    ...
    if (caption.length > 0) {
      ...
      formData.append('api_user', process.env.MODERATION_API_USER);
      formData.append('api_secret', process.env.MODERATION_API_SECRET);
