...
import { ArrowUturnRightIcon, ChatBubbleBottomCenterIcon, EllipsisVerticalIcon, XMarkIcon } from '@heroicons/react/24/outline'
...
import { ableToShareFunction, addHomeSaveFunction, getRequests, deleteReplyFunction, removeHomeSaveFunction, fetchUserFeedPosts, 
  addHomeLikeFunction, removeLikeFunction, fetchComments, getProfileDetails, fetchPublicPostsExcludingBlockedUsers, 
  addCommentLike, fetchMorePublicPostsExcludingBlockedUsers, fetchUsernames, removeCommentLike} from '@/firebaseUtils'
...
function Posts({post, changeWidth}) {
  ...
  //console.log(user)
    ...
    /* useEffect(() => {
    // Call the utility function to set up the listener
    const unsub = getRequests(user.uid, setRequests);
...
const staticPosts = [{ post: '../assets/logo.png' }, { post: '../assets/loginlogo.png' }];
  useEffect(() => {
    const handleResize = () => {
      const width = window.innerWidth;
      if (width < 768) { 
        setMarginClassOne('mx-32');
        setMarginClassTwo('mx-24');
      } else {
        setMarginClassOne('mx-32');
        setMarginClassTwo('mx-24')
      }
